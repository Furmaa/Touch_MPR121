...
#define SOFTWARE_NAME "Sinewave+Turnwheel switchable DEMO"
...
#define INTENSITY_UPPER  160u
#define INTENSITY_LOWER  20u  

/** number of functions */
#define PROGRAMS 2u
/** function numbering */
#define SINEWAVE_FUNCTION 0u
#define TURNWHEEL_FUNCTION 1u

/*****************************************************************************/
/** Internal variables *******************************************************/

/** function switching - starts with program 0, sinewave */
uint8_t function = 0u;
bool function_switched = false;
...
// ON/OFF state flags - because it is modified in ISR needs to be defined as volatile! */
volatile bool turned_on = false; 
volatile bool turning = false; 
/** switching state flag - because it is modified in ISR needs to be defined as volatile! */
...
{
  /** function switched? */
  if (function_switched)
  {
    #ifdef _LOGGING_
    Serial.println("Turning off interrupted, function switched to ");
    Serial.print(function);
    #endif /** _LOGGING_ */
    function_switched = false;
  }
...
//   Get the currently touched pads
  ...
  if (turned_on && !turning) 
  {
    switch (function)
    {
    case SINEWAVE_FUNCTION:
      /** different light shows for electrodes 1...7 */

      /** 1st electrode: increasing light intensity */
      if ( (currtouched & _BV(1)) && !(lasttouched & _BV(1)) && !(currtouched & ~_BV(1)) ) 
      { //only the 1th electrode has been touched 
  ...
  /*       turning = true;
      ...
      else if ( (currtouched & _BV(2)) && !(lasttouched & _BV(2)) && !(currtouched & ~_BV(2)) ) 
      { //only the 2th electrode has been touched 
  ...
  // Let's avoid it!
  if (inwardsintensity >= (INTENSITY_UPPER + 10u))
  {
    inwardsintensity = INTENSITY_UPPER + 10u;
  ...
  if (outwardsintensity >= (INTENSITY_UPPER + 10u))
  {
    outwardsintensity = INTENSITY_UPPER + 10u;
...
/*     Serial print occupies ISR for too long! Enable only for debugging.
  ...
  if ((ticking == 0u) && turned_on && turning) 
  {
    if ((currtouched & _BV(0)) && switching)
    {
      turning = false;
      switching = false;
      function = (function >= PROGRAMS) ? 0u : (function + 1u);
      function_switched = true;
    }
    else 
    {
      turned_on = false;
  ...
  else if ((ticking == 0u) && !turned_on && turning && (blink_count == 0)) 
  {
    turned_on = true;
    turning = false;
