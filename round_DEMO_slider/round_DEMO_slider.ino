...
#define SOFTWARE_NAME "Slider DEMO"
...
//   Get the currently touched pads
  ...
  if (turned_on && !turning) {
    ...
    inwardsintensity = (uint16_t) (inwardsintensity * MULTIPLIER);
    ...
    outwardsintensity = (uint16_t) (outwardsintensity * MULTIPLIER);
  }
  ...
  else if (!turned_on && turning && (ticking == 0)) 
  { //while turning on...
    if ((blink_count % 2) == 0) {
      inwardsintensity = 255; 
      outwardsintensity = 255; 
